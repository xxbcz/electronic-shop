...
import com.example.electronicshop.filter.TVFilter;
...
public class TvSpecification implements Specification<TvEntity> {
    ...
    private final TvFilterRequest filterCriteria;

    @Override
    public Predicate toPredicate(@NotNull Root<TvEntity> root, @NotNull CriteriaQuery<?> query,
                                 @NotNull CriteriaBuilder criteriaBuilder) {
        List<Predicate> predicates = new ArrayList<>();
        if (nonNull(filterCriteria.getFilter())) {

            TVFilter filter = filterCriteria.getFilter();
            ...
            if (hasLength(filter.getTechnology())) {
                predicates.add(criteriaBuilder.equal(root.get("technology"), filter.getTechnology()));
            }
