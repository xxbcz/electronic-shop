...
import com.example.electronicshop.filter.VacuumCleanerFilter;
...
public class VacuumCleanerSpecification implements Specification<VacuumCleanerEntity> {

    private final VacuumCleanerFilterRequest filterCriteria;

    @Override
    public Predicate toPredicate(@NotNull Root<VacuumCleanerEntity> root, @NotNull CriteriaQuery<?> query,
        ...
        if (nonNull(filterCriteria.getFilter())) {
            ...
            if (hasLength(filter.getVolumeOfVacuum())) {
                predicates.add(criteriaBuilder.equal(root.get("volumeOfVacuum"), filter.getVolumeOfVacuum()));
            }
